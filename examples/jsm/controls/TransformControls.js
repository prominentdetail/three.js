...
const _transformEvent = { type: 'transform' };
...
*/
	...
	pointerMove( pointer ) {
		...
		if ( mode === 'translate' ) {
			...
			if ( this.offsetSnap ) {
				...
				for( var i=0; i<3; i++ ){
					...
					_tempVector2.copy( this._offset ).projectOnVector( _tempVector );
				...
				this._offset.copy( _tempVector3 );
			...
			if ( axis.search( 'XYZ' ) !== - 1 ) {
				...
				let startLength = this.pointStart.length();
				let endLength = this.pointEnd.length();
		...
		this.dispatchEvent( _transformEvent );
	...
	setOffsetSnap ( offsetSnap ) {

		this.offsetSnap = offsetSnap;
